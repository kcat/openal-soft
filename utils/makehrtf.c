...
#ifdef HAVE_HRTF_SOFA
    fprintf(ofile, " -S              Expect a AES SOFA file format\n");
#endif
}
...
int main(const int argc, const char *argv[])
{
 ...
 #ifdef HAVE_HRTF_SOFA
   if(sofa)  {
	    if(!ProcessDefinitionSofa(inName, outRate, fftSize, equalize, surface, limit, truncSize, model, radius, outFormat, outName))
        	return -1;
    }
    else 
#endif
