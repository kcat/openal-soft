    ...
    uint frac, const uint increment, const std::span<float> dst)
{
    ...
    {
        const auto pos0 = as_unsigned(_mm_extract_epi32(pos4, 0));
        const auto pos1 = as_unsigned(_mm_extract_epi32(pos4, 1));
        const auto pos2 = as_unsigned(_mm_extract_epi32(pos4, 2));
        const auto pos3 = as_unsigned(_mm_extract_epi32(pos4, 3));
        ASSUME(pos0 <= pos1); ASSUME(pos1 <= pos2); ASSUME(pos2 <= pos3);
        ...
        const auto pi0 = as_unsigned(_mm_extract_epi32(pi4, 0));
        const auto pi1 = as_unsigned(_mm_extract_epi32(pi4, 1));
        const auto pi2 = as_unsigned(_mm_extract_epi32(pi4, 2));
        const auto pi3 = as_unsigned(_mm_extract_epi32(pi4, 3));
