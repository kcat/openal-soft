...
struct MakeHannWindow {
    alignas(16) std::array<float, N> mData{};
    ...
    MakeHannWindow() noexcept
    {
        ...
        std::ranges::transform(std::views::iota(0u, unsigned{N/2}), mData.begin(),
            [](unsigned const i) -> float
        {
            const auto val = std::sin((i+0.5) * scale);
            return static_cast<float>(val * val);
        });
        std::ranges::copy(mData | std::views::take(N/2), mData.rbegin());
    }
};


export template<std::size_t N>
inline const auto gHannWindow = MakeHannWindow<N>{};
