...
/*
 * `default_jack_error_callback' is exported by JACK1, while in JACK2
 * the callback is reset to the default by passing a NULL to
 * `jack_set_error_function'.  As such, after loading this symbol, passing
 * the resulting pointer to `jack_set_error_function' will reset the error
 * callback regardless of the version of JACK used.
 */
void (*pdefault_jack_error_callback)(const char *);
...
#elif defined(HAVE_DEFAULT_JACK_ERROR_CALLBACK)
extern void default_jack_error_callback(const char *);
#else
#define default_jack_error_callback NULL
#endif
...
#define JACKLIB "libjack.so.0"
        ...
        pdefault_jack_error_callback = GetSymbol(jack_handle, "default_jack_error_callback");
